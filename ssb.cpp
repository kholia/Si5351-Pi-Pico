...
void ssb_state_change(uint8_t state)
{
  if (state == 0)
  {
    if (ssb_active != 0)
    {
      ssb_active = 0;
      transmit_pwm_mode(0);
      sleep_ms(3); // wait for any interrupt to finish
  ...
  } else
  {
    if (ssb_active == 0)
    {
      // if (IS_SSB_PROTOCOL(ssbs.protocol))
      if (1)
      {
        muteaudio_set(1);
...
const uint8_t amplitude_table[256] =
{
...
void ssb_interrupt(int16_t sample)
{
  ...
  if (!ssb_active) return;

  ssbs.cumulative_sample += sample;
  if ((++ssbs.sampling_clock) < SAMPLING_CLOCK)
   return;

  ssbs.sampling_clock = 0;

  for (uint8_t i=0;i<SSB_FIR_LENGTH-1;i++)
    ssbs.ssb_fir_buffer[i] = ssbs.ssb_fir_buffer[i+1];

  ac = ssbs.cumulative_sample * 2;
  ac += ssbs.lpf_z1;              // LPF
  ssbs.lpf_z1 = (ssbs.cumulative_sample - (ssbs.lpf_z1 * 4) + ssbs.lpf_z1) / (3 + 1);  // guess that (3+1) is to make this an affine sum
  ssbs.dc_level = (ac + (ssbs.dc_level * 4) - ssbs.dc_level) / (3 + 1);
  ssbs.ssb_fir_buffer[SSB_FIR_LENGTH-1] = (ac - ssbs.dc_level) / 2;

  in_val = ssbs.ssb_fir_buffer[SSB_FIR_LENGTH/2-1];
  quad_val = ( (ssbs.ssb_fir_buffer[0] - ssbs.ssb_fir_buffer[14]) * 2 +
               (ssbs.ssb_fir_buffer[2] - ssbs.ssb_fir_buffer[12]) * 8 +
               (ssbs.ssb_fir_buffer[4] - ssbs.ssb_fir_buffer[10]) * 21 +
               (ssbs.ssb_fir_buffer[6] - ssbs.ssb_fir_buffer[8]) * 15) / 128
           + (ssbs.ssb_fir_buffer[6] - ssbs.ssb_fir_buffer[8]) / 2;
  ...
  magnitude = (magnitude * ssbs.gain) >> 3;
  ...
  ssbs.magnitude = magnitude;

  phase_difference = phase -  ssbs.previous_phase;
  ssbs.previous_phase = phase;   // calculate phase difference to get instantaneous frequency of signal
  ...
  {
    ssbs.previous_phase = phase + MAX_PHASE - phase_difference;
  ...
  printf("F -> %d\n", frequency);
  /* if (ssbs.protocol == PROTOCOL_LSB) // Fix this - Dhiru
    frequency = -frequency; */
  set_frequency_offset(frequency);

  ssbs.cumulative_sample = 0;
}
